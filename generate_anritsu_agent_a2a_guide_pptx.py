...
def build():
    ...
    text(slide, 0.78, 1.15, 11.4, 0.90, "KM Central Agent × Test Agents", 34, C["white"], True)
    text(slide, 0.82, 2.10, 10.8, 0.56, "A2A 功能實作與整合指南", 23, "BFD7FF", True)
    text(slide, 0.84, 3.05, 10.2, 0.80,
         "讓 KM 中央 Agent 安全委派 Anritsu／Amarisoft Agent 執行測試，\n"
    ...
    text(slide, 1.08, 5.33, 10.9, 0.48,
         "核心原則：KM 集中協調；測試 Agent 獨立執行；儀器控制留在測試端；KM 核心服務不重寫。",
    ...
    card(slide, 0.72, 1.45, 3.75, 1.75, "角色", "KM = Central Orchestrator / Control Plane\nBridge = outbound A2A Client\n測試端 = 獨立 A2A Server / Execution Plane", "blue_fill", "blue", 15, 11)
    ...
    items = [
        ("新增", "獨立 A2A Server、Agent Card、Task executor、queue / reject、權限與狀態管理"),
    ...
    text(slide, 1.65, 5.82, 9.95, 0.35, "推薦鏈路：KM Control Plane → A2A → Test Execution Plane → MCP / Instrument → Excel → 既有 KB HTTP ingest", 12, C["navy"], True, PP_ALIGN.CENTER)

    # 4. Architecture
    slide = new_slide(prs, "目標架構：KM 中央協調，多測試 Agent 獨立執行", "邏輯上是父子階層；runtime、憑證、queue 與儀器控制彼此隔離", 4)
    # left
    card(slide, 0.55, 1.55, 2.25, 1.45, "使用者", "chat.html\n詢問或批准測試", "gray_fill", "navy", 14, 10)
    card(slide, 3.25, 1.55, 2.55, 1.45, "KM Central Agent", "OpenClaw Orchestrator\n策略、批准、派工與追蹤", "blue_fill", "blue", 14, 9)
    card(slide, 6.35, 1.55, 2.55, 1.45, "Delegation Bridge", "獨立程序／預設關閉\noutbound A2A Client", "purple_fill", "purple", 14, 9)
    card(slide, 9.45, 1.25, 3.25, 1.20, "Anritsu Execution Plane", "獨立 Windows A2A Server\n儀器 lock + test executor", "orange_fill", "orange", 12, 9)
    card(slide, 9.45, 2.70, 3.25, 1.20, "Amarisoft Execution Plane", "獨立 A2A Server／credential\n相同契約、不同能力與 queue", "green_fill", "green", 12, 9)
    line(slide, 2.80, 2.28, 3.25, 2.28, C["blue"], 2, True)
    line(slide, 5.80, 2.28, 6.35, 2.28, C["purple"], 2, True)
    line(slide, 8.90, 2.28, 9.45, 1.85, C["orange"], 2, True)
    line(slide, 8.90, 2.28, 9.45, 3.30, C["green"], 2, True)
    ...
    text(slide, 8.97, 1.96, 0.53, 0.20, "A2A", 8, C["muted"], True, PP_ALIGN.CENTER)
    # lower paths
    card(slide, 9.55, 4.25, 2.75, 0.85, "儀器 / iperf", "各環境現有控制流程／固定 profile", "red_fill", "red", 12, 8)
    ...
    line(slide, 10.82, 3.90, 10.82, 4.25, C["orange"], 2, True)
    line(slide, 10.82, 5.10, 4.60, 5.25, C["teal"], 2, True)
    line(slide, 6.00, 5.78, 7.20, 5.78, C["line"], 1.6, True)
    line(slide, 9.40, 5.78, 10.15, 5.78, C["line"], 1.6, True)
    text(slide, 0.72, 6.62, 11.8, 0.25, "部署原則：KM 可隨時 submit；Agent 可 queued / reject。Bridge 與測試端皆獨立部署，不修改既有 KM 核心。", 10, C["navy"], True, PP_ALIGN.CENTER)
    ...
    text(slide, 1.08, 4.70, 11.1, 0.78, "Canonical correlation：context_id ↔ a2a_task_id ↔ run_id ↔ ingest_task_id（另存 file_hash）。\n四個欄位名稱必須固定並持久化；retry／重啟後仍可追蹤。A2A 只傳 metadata，不把 Excel 二進位塞入訊息。", 11, C["navy"], True, PP_ALIGN.CENTER)

    # 10. Task lifecycle
    slide = new_slide(prs, "Task 狀態與錯誤處理", "狀態要由普通程式碼維護，不交給 LLM 推測", 10)
    states = [
        ("submitted", "已提交", "KM 可隨時送出 request"),
        ("queued", "已排隊", "尚未取得 instrument lock"),
        ("working", "執行中", "已取得 lock／控制儀器"),
        ("completed", "A2A 工作終止", "依 artifact 查看三種狀態"),
        ("rejected", "拒絕", "busy／policy／capacity reason"),
        ("failed / canceled", "失敗／取消", "保留 error、cleanup、log"),
    ]
    for i, (code, name, body) in enumerate(states):
        x = 0.62 + (i % 3) * 4.18
        y = 1.45 + (i // 3) * 1.65
        accent = "green" if code == "completed" else "red" if code in {"rejected", "failed / canceled"} else "blue"
    ...
    text(slide, 1.08, 5.23, 11.1, 0.82, "A2A completed 不等於 KB ingest completed。必須分別回報 test_status、report_status、ingest_status。\nKM 可隨時 submit，但 Agent 必須 queue、取得 lock 後執行，或以穩定 machine-readable reason reject。重試不得重跑實體測試。", 10, C["orange"], True, PP_ALIGN.CENTER)
    ...
    rows = [
        ...
        ("KB 攝入失敗", "查 task status / retry", "分開顯示 test/report/ingest status"),
    ...
    text(slide, 0.82, 6.55, 11.65, 0.30, "context_id、a2a_task_id、run_id、ingest_task_id、file_hash 必須一起寫入 audit / local state。", 10, C["navy"], True, PP_ALIGN.CENTER)
    ...
    risks = [
        ...
        ("P1", "Excel / ingest 部分成功", "KB 顯示錯誤完成狀態", "三狀態 + reconciliation", "orange"),
    ...
    slide = new_slide(prs, "風險三：Excel、結果與 KB 攝入不一致", "測試、報告、攝入是三個獨立生命週期", 17)
    ...
    rect(slide, 0.82, 5.66, 11.65, 0.70, C["orange_fill"], C["orange"], True, 1.0)
    text(slide, 1.05, 5.78, 11.2, 0.44, "A2A artifact：context_id、a2a_task_id、run_id、ingest_task_id、report_name、file_hash、test_status、report_status、ingest_status。", 10, C["orange"], True, PP_ALIGN.CENTER)
    ...
    text(slide, 1.05, 5.78, 11.2, 0.44, "最低 audit：timestamp、caller、profile_id、context_id、a2a_task_id、run_id、ingest_task_id、三種 status、error_code、file_hash。", 9, C["navy"], True, PP_ALIGN.CENTER)

    # 20. Implementation phases
    slide = new_slide(prs, "實作分階段", "先驗證通訊，再逐步開放真實儀器操作", 20)
    phases = [
        ("Phase 0", "Mock", "Mock A2A server\n完整 task lifecycle\n不碰真實儀器", "blue"),
        ("Phase 1", "Dry-run", "Agent Card / schema\npolicy / correlation\n不控制儀器", "purple"),
        ("Phase 2", "Anritsu pilot", "單一 allowlisted profile\n真實 iperf + Excel\n既有 ingest", "orange"),
        ("Phase 3", "Amarisoft", "獨立 A2A Server\n獨立 credential / queue\n相同契約", "teal"),
        ("Phase 4", "Concurrency", "queue / reject / lock\n重啟與網路中斷\n雙環境並行", "green"),
    ]
    for i, (phase, head, body, accent) in enumerate(phases):
        ...
        text(slide, x + 0.15, 2.83, 1.82, 1.45, body, 9, C["ink"], False, PP_ALIGN.CENTER)
        if i < len(phases) - 1:
            line(slide, x + 2.12, 3.33, x + 2.55, 3.33, C["line"], 1.8, True)
    rect(slide, 0.82, 5.45, 11.65, 0.90, C["gray_fill"], C["line"], True, 1.0)
    text(slide, 1.08, 5.58, 11.1, 0.62, "每階段 gate：timeout / cancel / retry / idempotency、三狀態、audit 與 rollback 都要通過。\n任何失敗不得影響既有手動測試，以及 KM chat、search、ingest、Neo4j / Qdrant。", 10, C["navy"], True, PP_ALIGN.CENTER)

    # 21. Acceptance checklist
    slide = new_slide(prs, "Anritsu Agent 驗收清單", "完成下列項目才算可交付 KB 整合", 21)
    checks = [
        "Agent Card 可透過 HTTPS 取得，沒有洩漏秘密或內部路徑",
        "KM Bridge 可發送 A2A job，Anritsu 回傳 context_id 與 a2a_task_id",
        "只接受 allowlisted profile，不接受任意 command",
        "KM 可隨時 submit；busy 時可 queued 或以穩定 reason rejected",
        "context_id、a2a_task_id、run_id、ingest_task_id 可持久追蹤",
        "Excel 仍由既有 /api/upload/ingest 上傳，不新增第二套 uploader",
        "KM_Metadata 與 strict headers 通過驗證，duplicate/conflict 行為正確",
        "test_status、report_status、ingest_status 分離；A2A completed 不混同 ingest completed",
        "Windows agent 重啟後不會遺失未完成 task 或 audit 記錄",
        "手動測試及 KM chat/search/ingest/health 在 A2A 關閉或故障時仍正常",
        "Amarisoft 依相同契約獨立部署，使用不同 endpoint、credential 與 queue",
        "Anritsu／Amarisoft 並行時不交叉鎖定儀器，也不混用 run 或攝入資料",
    ]
    for i, item in enumerate(checks):
        x = 0.82 + (i % 2) * 6.00
        y = 1.38 + (i // 2) * 0.70
        rect(slide, x, y, 5.55, 0.50, C["green_fill"], C["green"], True, 0.8)
        text(slide, x + 0.10, y + 0.06, 0.32, 0.32, "✓", 13, C["green"], True, PP_ALIGN.CENTER)
        text(slide, x + 0.52, y + 0.06, 4.82, 0.32, item, 8, C["ink"], True)
    ...
    text(slide, 0.82, 6.52, 11.65, 0.42, "先提交 Agent Card、schema、mock / dry-run log、queue/reject 與 rollback 證據；主 Agent 核准後才開放真實儀器。", 10, C["navy"], True, PP_ALIGN.CENTER)
