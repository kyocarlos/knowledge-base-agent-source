...
| KM-002 | RUNTIME_VALIDATED (lifecycle API and store visibility integrated) | durable registry + Qdrant/Neo4j visibility filters | disposable production-compatible stack with real registry, Qdrant, Neo4j, Celery and Search API | `/search` API; `chat.html` remains the user-facing follow-up | real versioned document; draft/ready/publish transitions; v2 supersession; injected v3 store failure; query Search after each state | draft invisible; published current revision searchable; prior revision superseded; failed new revision leaves prior current revision searchable | RUNTIME_VALIDATED; USER_VISIBLE_VALIDATED pending | `docs/km002-real-system-validation.md` |
