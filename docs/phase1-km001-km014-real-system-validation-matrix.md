...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| KM-001 | INTEGRATED | converter/chunker -> Qdrant + Neo4j | disposable production-compatible stack with real document | `chat.html` search (user-visible validation pending) | ingest a real versioned markdown document; inspect processed chunks, embedding/vector point and Neo4j node | deterministic `document_id/document_version/chunk_id`, non-empty chunks, 768-dimension vector, and matching package metadata in both stores | RUNTIME_VALIDATED | `docs/evidence/km001-real-runtime-20260901.json` |
