...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| KM-001 | INTEGRATED | converter/chunker -> Qdrant + Neo4j | disposable production-compatible stack with real document | real `/search` API (chat/search UI follow-up optional) | ingest a real versioned markdown document; query `/search` through FastAPI/Celery; inspect processed chunks, embedding/vector point and Neo4j node | deterministic `document_id/document_version/chunk_id`, non-empty chunks, 768-dimension vector, matching package metadata in both stores, and user-visible source identity | USER_VISIBLE_VALIDATED | `docs/evidence/km001-real-runtime-20260901.json` |
| KM-002 | RUNTIME_VALIDATED (lifecycle API and store visibility integrated) | durable registry + Qdrant/Neo4j visibility filters | disposable production-compatible stack with real registry, Qdrant, Neo4j, Celery and Search API | `chat.html` with isolated OpenClaw gateway | real versioned document; draft/ready/publish transitions; v2 supersession; injected v3 store failure; chat query after each user-visible state | draft invisible; published current revision searchable; prior revision superseded; failed new revision leaves prior current revision searchable; chat displays surviving revision | USER_VISIBLE_VALIDATED | `docs/km002-real-system-validation.md`; `docs/evidence/km002-user-visible-runtime-20260901.json` |
| KM-003 | DONE | existing `knowledge_revisions` lifecycle + `reingest_document_revision` | disposable production-compatible stack with real FastAPI, Celery, Redis, Qdrant, Neo4j and registry | approved alternative `/search` user-facing API entrypoint | publish v1; ingest v2; inject store visibility failure; verify v1 Search; retry v2; verify Qdrant/Neo4j identity and current pointer | failed v2 remains unpublished while v1 stays searchable; successful retry publishes v2 and supersedes v1 | USER_VISIBLE_VALIDATED | `docs/km003-real-system-validation.md`; `docs/evidence/km003-user-visible-runtime-20260902.json` |
| KM-004 | RUNTIME_VALIDATED (isolated runtime) | existing `knowledge_revisions` lifecycle + Qdrant/Neo4j visibility adapters | disposable production-compatible stack with real FastAPI image, SQLite registry, Qdrant and Neo4j | lifecycle transition API diagnostic plus current-revision search contract | inject Qdrant failure and Neo4j failure independently; inspect structured outcome, rollback, durable registry and both stores; retry success | either-store failure is identifiable and fail-closed; no PUBLISHED/current partial revision; previous current remains; retry makes both stores and registry current | RUNTIME_VALIDATED; USER_VISIBLE_VALIDATED PENDING | `docs/km004-real-system-validation.md`; `docs/evidence/km004-real-runtime-20260902.json` |
...
| KM-014 | INTEGRATED | separate TimescaleDB metric path | real TimescaleDB service and FastAPI metrics API | `/api/v1/metrics` metric query | write real metric rows keyed by `test_run_id`, query by time range and inspect package/document identity | numeric time-series stored/queryable independently from report registry | RUNTIME_VALIDATED; USER_VISIBLE_VALIDATED | `docs/km014-timeseries-final-completion-review.md`; `docs/evidence/km014-timeseries-runtime-20260903.json` |
