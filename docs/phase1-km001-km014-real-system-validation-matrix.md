...
| KM-003 | IMPLEMENTED + INTEGRATED | existing `knowledge_revisions` lifecycle + `reingest_document_revision` | disposable production-compatible stack with real FastAPI, Celery, Redis, Qdrant, Neo4j and registry | `chat.html` search | publish v1; ingest v2; inject store visibility failure; verify v1 Search; retry v2; verify Qdrant/Neo4j identity and current pointer | failed v2 remains unpublished while v1 stays searchable; successful retry publishes v2 and supersedes v1 | RUNTIME_VALIDATED; USER_VISIBLE_VALIDATED PENDING | `docs/km003-real-system-validation.md`; `docs/evidence/km003-real-runtime-20260901.json` |
