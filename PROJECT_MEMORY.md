...
- 2026-09-03 Excel image chunk fix: canonical report ingestion now carries embedded Excel image references through enrichment, source sidecar metadata, and chunking. PR #45 branch `phase1-excel-image-chunks-20260903`, latest `b92e743`. Candidate image `kb-wp1-release:excel-image-chunks-20260903-r3` was validated in an isolated real FastAPI/Celery/Redis/PostgreSQL/Qdrant/Neo4j runtime. A real XLSX upload completed ingestion; `/admin/chunks` browser validation showed 5 image cards, 5 image refs, HTTP 200 assets, and zero console errors. Candidate containers were removed. Production was not modified; PR review/merge and authorized release deployment remain required for permanent Production behavior.
