...
# 2026-09-21 Patty review fix batch

- Read `/home/da40_ai_gb10/knowledge-base/AI_KM_Phase1_修改清單_20260921.xlsx` and created PR #59 from PR #58 head on branch `agent/km-phase1-patty-review-fixes-20260921`.
- Implemented KM-owned foundations: `ai_km_knowledge_v1` validation, stable document/version/chunk identity, draft-by-default Qdrant lifecycle, stage/commit/rollback coordinator, receiver atomic claim/lease/recovery and strict processor result semantics, parser signature registry, image watch allowlist, DOCX/PPTX structure enrichment, Neo4j provenance identity, fail-closed Qdrant errors, and explicit isolation of CSIT-originated documents from legacy KM business review.
- Evidence: isolated full `tests/` run `56 passed`; GitHub PR #59 contract check passed for head `8eebd83`. No real CSIT integration, Production secret/database/deployment, or joint contract freeze was performed. Remaining workbook P0/P1 items require further KM implementation or Patty/CSIT decisions, especially formal S2S/ACL/ID mapping, OCR/Vision provider, atomic cross-store runtime UAT, TimescaleDB, and Report Center.

# 2026-09-24 Simulated CSIT notification full-flow validator

- Continued only in isolated clone `/tmp/km-csit-formal-flow-AmHP9y/knowledge-base`, branch `agent/km-csit-simulated-formal-flow-20260923`; canonical dirty checkout and Production baseline were not changed.
- Added `scripts/run_csit_notification_e2e.py` and `docker-compose.csit-e2e.yml`.  The tool synthesizes an XLSX and temporary sender/reviewer credentials, then uses the real KM receiver, durable receipt, Celery ingest, canonical parser, Knowledge Package, Neo4j, Qdrant, TimescaleDB and sources-only Search path.  It never contacts CSIT and does not directly insert downstream records.
- Fixed genuine runtime gaps encountered by the exercise: Timescale migrations were absent from the application image; lifecycle-aware sidecar metadata was being downgraded to draft/non-current in vector ingest; raw Search sources omitted document/checksum provenance; and concurrent Qdrant collection creation could incorrectly disable one worker.
- Latest isolated evidence is persisted at `docs/evidence/km-csit-notification-e2e-20260924.json`: receipt completed, duplicate accepted, conflicting event rejected, one Timescale summary, two Search sources with run/checksum provenance, and Qdrant/Neo4j/Timescale residual counts all zero.  Local runtime command also passed 39 focused tests plus compile and compose validation.
- Commit `0d93203ba18f0edf5d961ee43b3cc4fa534a431a` was pushed to open PR #62. GitHub workflow `KM CSIT notification receiver` run `35963746091` was queued when last checked; treat its final status as requiring a refresh. This is KM-only isolated evidence, not real CSIT S2S, Production deployment, or Patty notification.
